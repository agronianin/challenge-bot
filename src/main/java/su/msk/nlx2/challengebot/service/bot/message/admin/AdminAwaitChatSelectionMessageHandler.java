    ...
    public void handle(MessageHandlerContext context) {
        if (context.message().chatShared() == null) {
            messageSender.sendText(
                    ...
                    botMessages.text(context.locale(), "challenge.create.repeat_select_chat"),
                    adminKeyboardFactory.selectChatKeyboard(context.locale())
            );
            return;
        }
        if (context.message().chatShared().requestId() != AdminKeyboardFactory.REQUEST_ID_SELECT_CHAT) {
            messageSender.sendText(
                    context.privateChatId(),
                    botMessages.text(context.locale(), "challenge.create.invalid_chat_selection"),
                    adminKeyboardFactory.selectChatKeyboard(context.locale())
            );
            return;
        }
        if (challengeAdminService.hasActiveOrScheduledProgram(context.message().chatShared().chatId())) {
            ...
            messageSender.sendText(
                    context.privateChatId(),
                    botMessages.text(context.locale(), "challenge.create.already_exists"),
        ...
        String chatTitle = context.message().chatShared().title() != null
                ? context.message().chatShared().title()
                : "chat " + context.message().chatShared().chatId();
        session.setChatTitle(chatTitle);
        session.setStartDate(LocalDate.now().plusDays(1));
        session.setStep(ConversationStep.CREATE_CHALLENGE_AWAIT_DAYS_TOTAL);
        messageSender.sendText(
                context.privateChatId(),
                botMessages.text(context.locale(), "challenge.create.start_date_auto", session.getStartDate()),
                adminKeyboardFactory.cancelOnly(context.locale())
        );
        messageSender.sendText(
                context.privateChatId(),
                botMessages.text(context.locale(), "challenge.create.ask_days_total"),
                adminKeyboardFactory.cancelOnly(context.locale())
        );
    }
}
